...
func NewFileSystem() *FileSystem {
	root := &FSNode{
		...
		Owner:     "root",
		Group:     "root",
		Permissions: Permissions{Owner: 7, Group: 5, Other: 5},
		CreatedAt: time.Now(),
		ModifiedAt: time.Now(),
	}
...
func (fs *FileSystem) CreateFile(path string, content []byte) error {
	...
	newFile := &FSNode{
		...
		Owner:       "root",
		Group:       "root",
		Permissions: Permissions{Owner: 6, Group: 4, Other: 4},
		CreatedAt:   time.Now(),
		ModifiedAt:  time.Now(),
	}
...
func (fs *FileSystem) ChangeOwner(path string, owner string) error {
	fs.Mu.RLock()
	defer fs.Mu.RUnlock()
...
func (fs *FileSystem) ChangePermissions(path string, perms Permissions) error {
	fs.Mu.RLock()
	defer fs.Mu.RUnlock()
